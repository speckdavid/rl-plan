...
while [ $counter -le $3 ] 
do
    python3 $DIR/fast-downward.py $1 --search "eager(rl([tiebreaking([pdb(pattern=manual_pattern([0,1,2,3,4])),weight(g(),-1)]), tiebreaking([pdb(pattern=manual_pattern([0,2])),weight(g(),-1)])]),rl=true,rl_control_interval=$2)" || exit 1
